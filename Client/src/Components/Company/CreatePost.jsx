...
const CreatePost = ({setState}) => {
  ...
  const [data,setData] = useState({authorId:user.user._id,authorName:user.user.companyName,authorPhoto:user.user.logo, authorType:'company',content:' ',photo:' '})
  const [warnnig,setWaring] = useState(false)
  const [success,setSuccess] = useState(false)
  const [backdrop,setBackdropOpen] = useState(false)
  const handleOpen =()=>{
    setData({authorId:user.user._id,authorName:user.user.companyName,authorPhoto:user.user.logo, authorType:'company',content:'',photo:''})
    setOpen(true)
  }
  const handleClose = () =>{
    setOpen(false)
...
const handlePost = async ()=>{
  ...
  const Userphoto =`../../../Profile_Image/${user.user.logo}`
  return (
    <Main p={1}>
       <Section>
