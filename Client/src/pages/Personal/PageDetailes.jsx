...
const handleChange = (event, newValue) => {
  ...
  useEffect(() => {
    ...
    try{
     setLoading(true)
     const responce = await axios.get(`/pages/${pagesId}`)
     ...
     <Box sx={{backgroundColor:"#E7EBF0"}}>
      ...
      <img src="../../../Profile_Image/coverPhoto.png" style={{ width: '100%', height: 'auto',borderTopLeftRadius:'6px',borderTopRightRadius:'6px' }} />
       ...
       <Box sx={{mt:{md:8,xs:6}}} pl={2}>
            ...
            {  user.user.pages.includes(page._id)?
                <Buttons onClick={()=>CancleRequest(page._id)} startIcon={<CheckOutlinedIcon/>} variant='outlined'>{t("Following")}</Buttons>:
            <Buttons onClick={()=>SnedRequest(page._id)} variant='outlined'>{t("Follow")}</Buttons>}
        </Box>
